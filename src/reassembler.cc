...
{
    std::cerr << "################################# " << m_func_calld << "insert: " << data
    ...
    auto reapir_unassembler_overlap = [&](uint64_t index) {
        ...
        while (true)
        {
            ...
            if (first_index > next_data->index && first_index < next_data->index + next_data->data_string.size())
            {
                m_reassemble_buf.insert_after(
                    new_data_position,
                    {first_index + data.size(), next_data->data_string.substr(first_index + data.size() - 1)});

                next_data->data_string.erase(first_index - 1);
        ...
        while (true)
        {
            ...
            if (data_end_index > next->index && data_end_index < next->index + next->data_string.size())
            {
                next->index += data_end_index - next->index;
